...
def note_sequence_to_tokens(seq) -> str:
    ...
    msg1 = OscMessageBuilder(address="/track13")
    msg1.add_arg(output_data1)
    m1 = msg1.build()

    msg2 = OscMessageBuilder(address="/track14")
    msg2.add_arg(output_data2)
    m2 = msg2.build()

    msg3 = OscMessageBuilder(address="/track15")
...
def run(config_map, recv_msg): #def run(config_map)
      ...
      if not tensors:
        ...
        for i, ns in enumerate(config.data_converter.from_tensors(tensors)):
          note_seq.sequence_proto_to_midi_file(
              ...
              ns, output_dir + 'ukraine' + str(i) + '.mid')
        # print(
  ...
  for i, ns in enumerate(results):
    ...
    note_seq.sequence_proto_to_midi_file(ns, output_dir + 'ukraine' + str(i) + '.mid')
  
  print(results)

  midi = midi_file_to_note_sequence(output_dir + 'ukraine' + str(i) + '.mid')
...
def socket_receive():
  ...
  while True:
    try:
        ...
        s.connect((socket.gethostname(), 2010))
