...
def note_sequence_to_tokens(seq) -> str:
    ...
    msg1 = OscMessageBuilder(address="/track28")
    msg1.add_arg(output_data1)
    m1 = msg1.build()

    msg2 = OscMessageBuilder(address="/track29")
    msg2.add_arg(output_data2)
    m2 = msg2.build()

    msg3 = OscMessageBuilder(address="/track30")
...
def run(config_map, recv_msg): #def run(config_map)
  ...
  for i, ns in enumerate(results):
    ...
    note_seq.sequence_proto_to_midi_file(ns, output_dir + 'china' + str(i) + '.mid')
  
  print(results)

  midi = midi_file_to_note_sequence(output_dir + 'china' + str(i) + '.mid')
